...
export function Header() {
  ...
  return (
    <header className={`sticky top-0 z-50 transition-all duration-300 ${scrolled ? 'bg-background/90 backdrop-blur-xl border-b border-border/50 shadow-sm' : 'bg-transparent backdrop-blur-sm'}`}>
      <div className="container mx-auto px-4 max-w-6xl">
        <div className="flex items-center justify-between h-16">
          ...
          <div className="flex items-center gap-1">
            ...
            {user ? (
              <>
                <Link to="/notifications">
                  <Button variant="ghost" size="icon" className="h-8 w-8 rounded-lg relative">
                    ...
                    <span className="absolute top-1.5 right-1.5 w-1.5 h-1.5 bg-highlight rounded-full" />
                  </Button>
                </Link>
                <Link to="/help">
                  <Button variant="ghost" size="icon" className="h-8 w-8 rounded-lg">
                  ...
                  <DropdownMenuTrigger asChild>
                    <Button variant="ghost" size="icon" className="h-8 w-8 rounded-full ml-1">
                      <Avatar className="h-7 w-7">
                        ...
                        <AvatarFallback className="text-[10px] bg-primary/10 text-primary font-bold">{getUserInitials()}</AvatarFallback>
                      </Avatar>
                    </Button>
                  </DropdownMenuTrigger>
              ...
              <Link to="/auth">
                <Button size="sm" className="gap-1.5 rounded-lg ml-1 h-8 px-4 text-xs font-semibold">
            ...
            <Button variant="ghost" size="icon" className="md:hidden h-8 w-8 rounded-lg" onClick={() => setIsMobileMenuOpen(!isMobileMenuOpen)}>
        ...
        {/* Mobile Navigation */}
        {isMobileMenuOpen && (
          <nav className="md:hidden py-3 border-t border-border/30 animate-fade-in space-y-1">
            {navLinks.map((link) => (
              <Link
                key={link.path}
                to={link.path}
            ...
            {!user && (
              <Link to="/auth" onClick={() => setIsMobileMenuOpen(false)} className="block px-4 py-2.5 rounded-xl text-sm font-semibold text-primary hover:bg-muted/60">
                Sign In / Sign Up
              </Link>
            )}
          </nav>
